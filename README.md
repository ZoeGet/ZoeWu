...
**御坂小屋**是一个使用 Hexo + Butterfly 搭建的个人技术博客，主要记录 Linux 运维、计算机网络、嵌入式开发、Git / GitHub、Hexo 部署和开发工具排坑。

这个仓库只保存 GitHub Pages 发布后的静态网页文件，不保存完整 Hexo 源码。
...
- **Git / GitHub**：提交身份配置、提交信息规范、远程仓库、分支切换、初始化到推送流程、版本回滚等。
...
本仓库是公开发布仓库，只包含生成后的静态站点文件。

Hexo 源码、主题配置、文章源文件和构建环境建议保存在单独的私有仓库中。
